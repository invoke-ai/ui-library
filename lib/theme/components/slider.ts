...
const thumb = defineStyle((props) => {
  ...
  return {
    ...
    borderWidth: 3,
    _hover: {
      transform: orientation === 'vertical' ? 'translateX(-50%) scale(1.15)' : 'translateY(-50%) scale(1.15)',
      transition: 'transform 0.1s',
      _active: {
        transform: orientation === 'vertical' ? 'translateX(-50%) scale(1.22)' : 'translateY(-50%) scale(1.22)',
